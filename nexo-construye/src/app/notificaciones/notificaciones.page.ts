...
import {IonContent, IonTitle, IonToolbar,IonIcon, IonButtons, IonBackButton, IonSpinner,IonButton, IonBadge, ToastController,} from '@ionic/angular/standalone';
...
@Component({
  ...
  standalone: true,
  imports: [IonContent,IonTitle, IonToolbar, IonIcon, IonButtons, IonBackButton,
  IonSpinner, IonButton, IonBadge,CommonModule]
})
export class NotificacionesPage implements OnInit, OnDestroy {
 ...
 getTitulo(tipo: string): string {
  switch (tipo) {
    ...
    case 'cancelacion_solicitada': return 'Solicitud de cancelación';
 ...
 getIcono(tipo: string): string {
  ...
  switch (tipo) {
    ...
    case 'mensaje_nuevo':          return '#38bdf8';
  ...
  switch (tipo) {
    ...
    case 'mensaje_nuevo':          return 'rgba(56,189,248,0.08)';
 ...
 async aceptarReserva(noti: any) {
  ...
  this.reservacionService.confirmarReservacion(noti.reservacionId).subscribe({
    next: async () => {
      this.reservacionService.getReservacionById(noti.reservacionId).subscribe({
        next: (reservacion) => {
          this.conversacionService.iniciarConversacion({
            clienteId: reservacion.cliente.toString(),
            profesionalId: reservacion.profesional.toString(),
  ...
  const id = this.usuario?._id || this.usuario?.id;
  if (!id) return;
  this.notificacionService.eliminarTodasNotificaciones(id).subscribe({
    next: async () => {
      await this.mostrarToast('Notificaciones eliminadas', 'medium');
      this.notificaciones = [];
    },
    error: async () => await this.mostrarToast('Error al limpiar', 'danger')
  });
}

async confirmarLimpiar() {
  const alert = await this.alertControl.create({
    header: 'Eliminar notificaciones',
    message: '¿Estás seguro de que deseas eliminar todas las notificaciones?',
    buttons: [
      { text: 'Cancelar', role: 'cancel' },
      { text: 'Eliminar', role: 'confirm', handler: () => this.limpiarTodas() }
    ]
  });
  await alert.present();
}

irAlChat(notificacion: any) {
  if (!notificacion.reservacionId) return;
  this.router.navigate(['/chat', notificacion.reservacionId], {
    queryParams: { nombreOtro: notificacion.mensaje.split(' ')[0] }
  });
}
}